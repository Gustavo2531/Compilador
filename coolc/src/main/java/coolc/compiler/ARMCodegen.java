...
public class ARMCodegen extends DepthFirstAdapter implements CodegenFacade {
	...
	class SweepConstants extends DepthFirstAdapter {
		...
		public void inAStrExpr(AStrExpr node) {
			...
			st.addAggr("ints.{idx,value}", lidx++, s.length()+1);
			int int32size = ( (s.length() + 1 + 3) & ~0x03 )/4 ;
			st.addAggr("strings.{idx,size,sizeIdx,value}", lidx, int32size+3, lidx-1, Util.escapeString(s));
			literalIdx.put(node, lidx++);
		}
		@Override
		public void inAIntExpr(AIntExpr node) {
			st.addAggr("ints.{idx,value}", lidx, node.getIntConst().getText());
			literalIdx.put(node, lidx++);
		}
	}
